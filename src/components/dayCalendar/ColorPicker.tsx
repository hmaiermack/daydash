    ...
    <Combobox defaultValue={props.value} onChange={props.onChange} refName={props.name} nullable disabled={props.disabled}>
        {({open}) => (
            ...
            */}
            ...
            />
            <Combobox.Button
            className={`w-10 h-10  ml-2 rounded-full focus:outline-none focus:shadow-outline inline-flex p-2 shadow text-white ${open ? 'hidden' : ''}`} style={{background: `${inputString}`}}>
